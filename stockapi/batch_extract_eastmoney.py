...
def main():
    ...
    if not os.path.exists(stocks_file):
        print(f"错误：找不到文件 {stocks_file}")
        print("请先运行 get_hk_ggt_stocks.py 获取港股通成分股列表")
        return
    
    # 读取CSV文件
    stocks_df = pd.read_csv(stocks_file, encoding='utf-8-sig')
    print(f"成功读取 {len(stocks_df)} 只港股通成分股")
    
    # 确保股票代码列是字符串类型，保留前导零
    stocks_df['代码'] = stocks_df['代码'].astype(str).str.zfill(5)
    print("已确保股票代码格式正确（保留前导零）")
    
    # 限制处理的股票数量
    if args.limit is not None and args.limit > 0:
        stocks_df = stocks_df.head(args.limit)
        print(f"根据参数限制，将只处理前 {args.limit} 只股票")
    ...
    for index, row in stocks_df.iterrows():
        ...
        while retry_count < args.retry and not success:
            ...
            try:
                ...
                if result.returncode == 0:
                    print(f"成功提取 {stock_code} {stock_name} 的数据")
                    success = True
                    success_count += 1
                else:
                    print(f"提取 {stock_code} {stock_name} 的数据时出错:")
                    print(result.stderr)
                    retry_count += 1
                
            except Exception as e:
                print(f"处理 {stock_code} {stock_name} 时发生异常: {str(e)}")
                retry_count += 1
        ...
        if not success:
            error_count += 1
            print(f"在 {args.retry} 次尝试后仍然无法提取 {stock_code} {stock_name} 的数据")
