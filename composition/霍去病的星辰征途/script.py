...
while "所有章节都存在" not in check_chapters(max_chapter):
    ...
    while check_content(content_chapter, last_15_lines):
        ...
        # 重新组合成字符串
        result = '\n'.join(last_15_lines)
        chapter_writer_new(plot=plot, last_chapter=result, outline=outline, character_document=character_document, wordage=2500, title=title, style="大开脑洞，英雄气概，有勇有谋", model=model)
