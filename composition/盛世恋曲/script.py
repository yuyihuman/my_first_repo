...
while "所有章节都存在" not in check_chapters_new(max_chapter):
    ...
    # with open(volume, 'r', encoding='utf-8') as file:
    #     outline_volume = file.read()
    outline_volume = f'{title}\n{get_chapter_content(title)}'
    outline_volume += f'{next_title}\n{get_chapter_content(next_title)}'
    content_chapter = ""
    result = ""
    while check_content(content_chapter, result):
        try:
        ...
        # 生成 15 到 19 之间的随机整数
        random_num = random.randint(25, 29)
        # 提取最后 15 行
        last_15_lines = lines[-random_num:-1]
        # 重新组合成字符串
        filtered_lines = [line for line in lines if "篇章正文开始" not in line and "篇章正文结束" not in line]
        result = '\n'.join(filtered_lines)
        output = ""
        while not output:
            output = chapter_writer(outline_volume=outline_volume, last_chapter=result, outline=outline, character_document=character_document, wordage=2500, title=title, model=model)
